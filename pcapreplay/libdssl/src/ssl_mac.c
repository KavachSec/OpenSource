						 ...
						 u_char* data, uint32_t len, u_char* mac )
{
	...
	EVP_MD_CTX_reset(md_ctx);
        ...
        printf("--->kiran called: ssl3_calculate_mac\n");
						 ...
						 u_char* data, uint32_t len, u_char* mac )
{
	...
	mac_size = EVP_MD_size( md );
	//HMAC_CTX_init( hmac ); //kiran - not defined
	...
	HMAC_CTX_reset( hmac ); //kiran check if - where it is freed HMAC_CTX_free 
	
	DEBUG_TRACE_BUF("kiran: tls1_calculate_mac: mac", mac, mac_size);
